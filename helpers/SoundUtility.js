...
export const loadSound = async (soundName) => {
  try {
    const audioSource = getSoundPath(soundName);
...
export const playSound = async (soundName, loop = false) => {
  try {
    const audioSource = getSoundPath(soundName);

    // Create a new Sound object
    const { sound } = await Audio.Sound.createAsync(audioSource, { shouldPlay: true, isLooping: loop });
    } catch (error) {
    console.log('Error playing sound:', error);
  }
};
